...
- **Google Maps** - real satellite/street tiles for the Live map's Kyiv close-up view only. Fetched from the same tile endpoint the Google Maps website itself uses, not the official billed Maps Tiles API - fine at this app's current scale, credited in-app and here, but worth revisiting if that ever changes.
