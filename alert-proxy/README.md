...
| Today-stats | Polled every 5 min, 1 call/cycle in steady state - the alerts.in.ua fallback call fires whenever UkraineAlarm's `/alerts/dateHistory` is down. As of 2026-09-15, confirmed by direct testing, this endpoint flaps between working and returning 401 with an empty body, unrelated to our token (which works fine for the main `/alerts` and `/alerts/status` endpoints) - looks like a UkraineAlarm-side reliability issue on history endpoints specifically, the same class of problem already known for `/alerts/regionHistory`. The proxy persists a consecutive-failure count in Durable Object storage (survives hibernation) and backs off to a 30-minute retry once it fails 3 times in a row for the day, instead of retrying every 2 minutes forever. | ~288 |
